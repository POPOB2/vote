...
<body>
    ...
    <div id="container">
    <?php
    // ------------------------------這裡產生一個問題  無法判斷網頁是否存在  所以需要增加一個if去判斷網頁的存在與執行的結果------------------------------
    // ------------------------------------------------------------------------------------------------------------------------------------------------
    if(isset($_GET['do'])){// 判斷網址有無GET到do參數, 若有do這段文字參數  執行以下
        ...
        include $file;// 此時 $file= "./back/add_vote.php"  (依照需求可以將上述的.php改為.html 就可以使用html執行一樣的事情)
        // *這裡注意 : 是用include是將內容導入,而非導出 所以這裡的include $file; 將add_vote.php的內容導入到back.php
        }else{// 若無 顯示原本的新增投票與投票列表
    ?>        
